...
let ProductTechnologyModel = {
  ...
  addProductTechnology: async (productId, technologyId) => {
    try {
      console.log(
        `productId thêm: ${productId}, technologyId: ${technologyId}`
      );
  ...
  getTechnologiesByProductId: async (productId) => {
    try {
      const query = `SELECT t.name ,t.category_id,t.id FROM technologies t 
  ...
  deleteProductTechnology: async (productId, technologyId) => {
    console.log(`productId: ${productId}, technologyId: ${technologyId}`);
    try {
